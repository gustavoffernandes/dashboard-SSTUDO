...
import { methodologyLabel, normalizeMethodology } from "@/lib/methodology";
...
export default function Companies() {
  ...
  const branchMap = new Map<string, { id: string; cnpj: string; city: string; name: string; sector: string; employee_count: number | null; count: number; sectors: CompanySector[]; priority: 0 | 1; methodologies: Set<string> }>();
  ...
  configs.forEach((c: any) => {
    ...
    if (branchMap.has(key)) {
      ...
      if (!isPlaceholder) { current.count++; current.methodologies.add(normalizeMethodology(c.methodology)); }
      ...
      if (!isPlaceholder) methodologies.add(normalizeMethodology(c.methodology));
      branchMap.set(key, { id: c.id, cnpj, city, name: c.company_name, sector: c.sector || "", employee_count: c.employee_count || null, count: isPlaceholder ? 0 : 1, priority, sectors: parsedSectors, methodologies });
    }
  });
  ...
  branchMap.forEach((val, key) => {
    companies.push({
      ...
      sector: val.sector, employee_count: val.employee_count,
      formMethodologies: Array.from(val.methodologies).sort(),
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          {company.formMethodologies.map(m => (
                            <span key={m} className="inline-flex items-center rounded-full bg-accent px-2 py-0.5 text-[10px] font-semibold text-accent-foreground uppercase tracking-wider">
                              {methodologyLabel(normalizeMethodology(m))}
                            </span>
                          ))}
                        </div>
